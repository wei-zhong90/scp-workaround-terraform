...
variable "project_name" {
  description = "The unique key for backend state"
  type = string
  default = "some unique state key name"
}

variable "state_name" {
  description = "The name to use for S3 buckets, since names are globally unique."
  type        = string
  default     = "null"
}
...
variable "profile" {
  description = "the profile to use for aws account credentials"
  type        = string
  default     = "default"
}
